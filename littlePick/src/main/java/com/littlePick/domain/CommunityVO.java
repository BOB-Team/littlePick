...
public class CommunityVO {
	...
	private int ordercount;	//주문 리스트 개수 불러옴 count(*) ordercount 때문에 추가함
	private double avgstar; //리뷰 평점 계산
	...
	public void setAvgstar(double avgstar) {
		...
		private String user_address2; // 주소
		private String user_address3; // 주소
		...
		public String review_date;
			...
			//파일 업로드 관련
			//유저이미지
			MultipartFile userfile;	// write.jsp에 파일첨부시 name="file"과 동일한 변수명
			...
			public void setUserfile(MultipartFile userfile) {
				...
				// 업로드 파일 접근
				if(! userfile.isEmpty()){
					this.user_image = userfile.getOriginalFilename();
			...
			//리뷰이미지  review_image
			MultipartFile reviewfile;	// write.jsp에 파일첨부시 name="file"과 동일한 변수명
			...
			public void setReviewfile(MultipartFile reviewfile) {
				...
				if(! reviewfile.isEmpty()){
					this.review_image = reviewfile.getOriginalFilename();
